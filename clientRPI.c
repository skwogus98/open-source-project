...
void userTask(char *hostname, int port, char *filename, int period)
{
 ...
 time_t ltime;
 time(&ltime);
 double curtime = (double)ltime;
​
 read_dht11_dat(curtime, filename, hostname, port, period);
}
​
...
​
// sensor관련 코드
void read_dht11_dat(double curtime, char *filename, char *hostname, int port, int period)
{
 char msg[MAXLINE];
